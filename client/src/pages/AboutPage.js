    ...
    <div style={{ paddingTop: 'var(--nav-height)', minHeight: '100vh' }}>
      ...
      <section style={{ background: 'linear-gradient(160deg, #0f172a 0%, #1e293b 100%)', padding: '80px 0', color: 'white' }}>
        <div className="container" style={{ textAlign: 'center' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,.1)', borderRadius: 99, padding: '6px 16px', marginBottom: 24, fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,.8)' }}>
            🏆 India's Largest Compliance Platform
          </div>
          <h1 style={{ fontSize: 'clamp(32px, 5vw, 56px)', fontFamily: 'var(--font-heading)', fontWeight: 800, color: 'white', marginBottom: 20, lineHeight: 1.15 }}>
            Simplifying Compliance<br />for Every Business
          </h1>
          <p style={{ fontSize: 18, color: 'rgba(255,255,255,.7)', maxWidth: 600, margin: '0 auto 40px', lineHeight: 1.7 }}>
            Since 2016, Jeeban & CO has been on a mission to make business registration and compliance effortless for every Indian entrepreneur.
          </p>
          <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link to="/services" className="btn btn-primary btn-lg">Explore Services</Link>
            <Link to="/contact" className="btn btn-lg" style={{ background: 'rgba(255,255,255,.1)', color: 'white', border: '1.5px solid rgba(255,255,255,.2)' }}>Contact Us</Link>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: '#1a56db', padding: '40px 0' }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 32 }} className="stats-bar">
            {[
              ...
              <div key={s.label} style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 36, fontWeight: 900, fontFamily: 'var(--font-heading)', color: 'white', lineHeight: 1 }}>{s.value}</div>
                <div style={{ fontSize: 13, color: 'rgba(255,255,255,.75)', marginTop: 6 }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="section" style={{ background: 'white' }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }} className="mission-layout">
            <div>
      ...
      <section className="section" style={{ background: '#f8fafc' }}>
        <div className="container">
          ...
          <div style={{ position: 'relative', maxWidth: 800, margin: '0 auto' }}>
            ...
            {MILESTONES.map((m, i) => (
              <div key={m.year} style={{ display: 'flex', justifyContent: i % 2 === 0 ? 'flex-start' : 'flex-end', marginBottom: 32, position: 'relative' }}>
                <div style={{ width: '45%', background: 'white', border: '1px solid #e2e8f0', borderRadius: 16, padding: 24, boxShadow: '0 2px 12px rgba(0,0,0,.05)' }}>
                  <div style={{ fontSize: 14, fontWeight: 800, color: '#1a56db', marginBottom: 4 }}>{m.year}</div>
                ...
                <div style={{ position: 'absolute', left: '50%', top: 24, width: 14, height: 14, borderRadius: '50%', background: '#1a56db', border: '3px solid white', transform: 'translateX(-50%)', boxShadow: '0 0 0 3px rgba(26,86,219,.2)' }} />
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="section" style={{ background: '#f8fafc' }}>
        <div className="container">
          <div style={{ textAlign: 'center', marginBottom: 40 }}>
            <div className="section-label" style={{ justifyContent: 'center' }}>Trust & Credentials</div>
            <h2 className="section-title">Recognised & Certified</h2>
          </div>
          <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', justifyContent: 'center' }}>
            {[
              { icon: '🏅', title: 'ISO 27001:2022', sub: 'Information Security' },
              { icon: '🔐', title: 'SSL Secured', sub: '256-bit Encryption' },
              { icon: '🏛️', title: 'MCA Registered', sub: 'eFiling Intermediary' },
              ...
              <div key={cert.title} style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: 14, padding: '20px 28px', textAlign: 'center', minWidth: 150 }}>
                <div style={{ fontSize: 32, marginBottom: 8 }}>{cert.icon}</div>
                <div style={{ fontSize: 14, fontWeight: 700, color: '#0f172a' }}>{cert.title}</div>
                <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }}>{cert.sub}</div>
              </div>
            ))}
          </div>
        </div>
